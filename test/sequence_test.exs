defmodule SequenceTest do
  ...
  test "#finite_lazy: is a lazy finite sequence with n terms " <>
    ...
    assert Enum.to_list(Sequence.finite_lazy(1, 1, 1)) == [{1, 1}]
    assert Enum.to_list(Sequence.finite_lazy(2, 1, 2)) == [{1, 1}, {2, 2}]
    assert Enum.to_list(Sequence.finite_lazy(4, 1, 4)) == [{1, 1}, {2, 2},
                                                           {3, 3}, {4, 4}]

    assert Enum.to_list(Sequence.finite_lazy(3, -1.5, 1.5)) == [{1, -1.5},
                                                                {2, 0.0},
                                                                {3, 1.5}]

    floater = Enum.to_list(Sequence.finite_lazy(16, -1.5, 1.5))
    assert floater |> List.first == {1, -1.5}
    assert floater |> List.last == {16, 1.5}
    assert floater |> Enum.count == 16

    big_floater = Enum.to_list(Sequence.finite_lazy(800, -1.5, 1.5))
    assert big_floater |> List.first == {1, -1.5}
    assert big_floater |> List.last == {800, 1.5}
